  ...
  var JshSlider = (function() {
    ...
    JshSlider.fn = JshSlider.prototype = {
      ...
      init : function(slider) {
        ...
        var boxTagArr = this.switchTagWrapper(this.stringToTag(this.idTag, this.subTag), true);
        
        // 图片个数
        this.count = boxTagArr.length;
         // 设置图片大小
        slider.width = parseInt(getStyle(boxTagArr[0], "width"));
        slider.height = parseInt(getStyle(boxTagArr[0], "height"));
        ...
        if(!!slider.switchBtn) {
          ...
          this.btnTagArr = this.switchTagWrapper(this.stringToTag(btnIdTag, btnSubTag), false);
        ...
        if ( slider.style == "left" ) {
          slider.totalWidth = parseInt(getStyle(boxTagArr[0], "width"));
        } else {
          this.ele.style.width = "960px";
          this.ele.style.height = "800px";
          this.ele.style.left = "0px";
          this.ele.style.overflow = "hidden";
          slider.totalWidth = parseInt(getStyle(boxTagArr[0], "height"));
        }
        ...
        if(!slider.mouseover) {
          that.mouseover(that.ele);
        }

        // 添加向左、向右按钮的mouseover、mouseout事件
        // 绑定向左向右事件
        if(slider.btnList && slider.btnList[0] && slider.btnList[1]) {
          that.mouseover(domJs.getId(slider.btnList[0]));
          that.mouseover(domJs.getId(slider.btnList[1]));

          that.click(domJs.getId(slider.btnList[0]), function() {
            that.clickBtn(that.current - 1, that.slider.btnList[1]);
          });
          that.click(domJs.getId(slider.btnList[1]), function() {
      ...
      stringToTag : function(idTag, subTag) {
      ...
      switchTagWrapper : function(arr, isSetId) {
        var j = 1;
        for(var i = 0, tag, len = arr.length; i < len; i++) {
          tag = arr[i];
          if(isSetId) {
            tag.setAttribute("id", this.sliderTimeStr + j);
          }
          if(tag.cssText) {
            tag.style.cssText = "z-index:" + j;
          } else {
            tag.setAttribute("style", "z-index:" + j);
          }
      ...
      initRightAction : function() {
        var arr = this.stringToTag(this.idTag, this.subTag);
